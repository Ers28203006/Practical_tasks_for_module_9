...
{
    class DVD : DataCarrier
    {
        public DVD(string storadgeDataName, string storargeModel, double amountOfMemory) :
           base(storadgeDataName, storargeModel, amountOfMemory){ }

        public double SpeedReadWrite { get; set; } = 1.32;

        public override void TimeToTransfer()
        {
            double timeToTransfer;
            timeToTransfer = ScopeOfAllInformation / SpeedReadWrite;
